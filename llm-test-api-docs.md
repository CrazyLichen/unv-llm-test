...
> 版本：v2.1
> 基于界面原型 `llm-test-prototype.html` 生成
> 更新日期：2026-07-26
...
| VideoPath | string \| null | 视频本地路径（Type=Video 时有值） |
| FrameInterval | int32 \| null | 抽帧间隔，单位秒（Type=Video 时有值） |
| Progress | object | 检测进度与统计，见 [Progress](#23-progress检测进度与统计) |
| CreatedAt | string | 创建时间 |

### 2.3 Progress（检测进度与统计）
...
| CompletedDetail | object | 已完成明细，见 [CompletedDetail](#231-completeeddetail已完成明细) |
| Pending | int32 | 待检测数 |

#### 2.3.1 CompletedDetail（已完成明细）

| 字段 | 类型 | 说明 |
|------|------|------|
| Detected | int32 | 已检出目标数 |
| DetectedDetail | object | 检出明细，见 [DetectedDetail](#232-detecteddetail检出明细) |
| NotDetected | int32 | 未检出目标数 |
| Failed | int32 | 检测失败数 |

#### 2.3.2 DetectedDetail（检出明细）
...
| Boxes | array | 检测框列表，见 [Box](#26-box检测框) |
...
- **URL**：`PATCH /api/model-configs/:id`
- **Content-Type**：`application/json`
...
| ImagePaths | string[] | Type=Image 时必填 | 图片本地路径列表 |
| VideoPath | string | Type=Video 时必填 | 视频本地路径 |
...
```json
{
  ...
  "Target": "消防通道占用物",
  "VideoPath": "D:/videos/firelane_cam_01.mp4",
  "FrameInterval": 2
}
```
...
1. 校验参数（ModelConfigId 是否存在、路径是否存在、必填项是否完整）
2. 创建任务记录，关联指定的模型配置，状态设为 `Pending`（等待中），进入后端任务排队队列
3. **Type=Image**：为每张图片创建 Image 记录，状态设为 `Pending`（待检测）
4. **Type=Video**：为视频创建占位记录，待任务开始执行后再异步抽帧生成帧 Image 记录
5. 后端按排队顺序依次执行任务，任务开始执行时状态变为 `Analyzing`（检测中），使用关联的模型配置调用大模型
6. 立即返回，不等待任务开始执行
...
查询任务下所有素材的检测结果。图片集任务和视频集任务的结果结构一致：图片集的素材由用户指定路径，视频集的素材由后端根据抽帧间隔自动抽帧生成，最终都是逐张图片调大模型检测，结果统一呈现。支持按素材 ID 查询单条或按检测状态分页筛选。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| ImagePath | string | 是 | 补录图片的本地路径 |

#### Request Example

```json
{
  "ImagePath": "D:/photos/missed_003.jpg"
}
```
...
| 3 | PATCH | `/api/model-configs/:id` | 更新模型配置 |
| 4 | DELETE | `/api/model-configs/:id` | 删除模型配置 |
| 5 | POST | `/api/model-configs/:id/test` | 测试模型连通性 |
| 6 | POST | `/api/tasks` | 创建任务（排队等待执行） |
| 7 | GET | `/api/tasks` | 获取任务列表（支持按 ID 查询单条） |
| 8 | DELETE | `/api/tasks/:id` | 删除任务（级联硬删除） |
| 9 | PATCH | `/api/tasks/:id` | 暂停/恢复任务 |
| 10 | GET | `/api/tasks/:id/images` | 获取任务分析结果（支持按素材 ID 查询单条） |
| 11 | POST | `/api/tasks/:id/images` | 补录漏报照片到素材集 |
| 12 | PATCH | `/api/tasks/:id/images/:imageId` | 标记素材误报/删除误报/恢复 |
...
```
  POST /api/tasks
   ...
   └──┬───┬───┘
      ...
      │   └────┬─────┘
      │        │
      │   PATCH { Status: "Analyzing" }  (仅暂停→可恢复)
      │        │
      │        ▼
   后端调度执行 ─────────────────────────────────┐
      │                                         │
...
│                                                                  │
│  POST /api/tasks                                                 │
│  { Type, ModelConfigId, Prompt, Target, ImagePaths/VideoPath }   │
│         │                                                        │
│         ▼                                                        │
│  ┌─────────────┐                                                │
...
│  └──────┬──────┘                    └─────────┘ └──────────┘ │败│ │
│         │                                          └─┬──────┘ │ │
│  PATCH { Status: "Analyzing" }                      FailReason  │ │
│         │                                                        │
│         ▼                                                        │
│    恢复检测，继续从中断处执行                                       │
│                                                                  │
...
│                                                                  │
│  POST /tasks/:id/images { ImagePath }                            │
│  → 后端将照片加入任务素材集，列表自动体现                           │
└──────────────────────────────────────────────────────────────────┘
```
